...
def random_get_Nway_Kshot(target_X, target_y,n_way,k_shot):
   ...
   # print("target_y shape:",target_y.shape)
    for i in range(n_way):
        # print()
        # print(k_shot)
        if len(np.where(target_y==i)[0].tolist()) == 0:
          continue
        choice = random.sample( np.where(target_y==i)[0].tolist(), k_shot)
        support_x.append(target_X[choice])
...
all_model_fn = sorted(glob('./model_weights_HAR_MH/*.pth'), key=cond)[-8:]
